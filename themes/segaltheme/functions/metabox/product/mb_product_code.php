...
function cd_mb_code_product_cb( $post )
{
	...
	$text   = isset( $values['mb_code_product'] ) ? $values['mb_code_product'][0] : '';
    ...
    ?>
    <p>
        <label for="mb_code_product"> Código: </label>
        <input size="" type="text" name="mb_code_product" id="mb_code_product" value="<?php echo $text; ?>" placeholder="eg. COD 46" />
    </p>
    <?php    
}
//guardar la data
...
function cd_mb_code_product_save( $post_id )
{
    ...
    if( isset( $_POST['mb_code_product'] ) )
        update_post_meta( $post_id, 'mb_code_product', wp_kses( $_POST['mb_code_product'], $allowed ) );
}